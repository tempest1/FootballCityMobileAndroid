...
import com.footballcitymobileandroid.BLL.Util.SystemUtil.TimeUtil;
...
import com.footballcitymobileandroid.alibaba.openIMUIDemo.LoginActivity;
import com.footballcitymobileandroid.openimui.common.Notification;
import com.footballcitymobileandroid.openimui.contact.ContactCacheUpdateListenerImpl;
import com.footballcitymobileandroid.openimui.contact.ContactOperateNotifyListenerImpl;
import com.footballcitymobileandroid.openimui.demo.DemoApplication;
...
public class LoginSampleHelper {
    ...
    public static  String APP_KEY = "23417173";
    ...
    public void initSDK_Sample(Application context) {
        ...
        if(type== TcmsEnvType.ONLINE || type == TcmsEnvType.PRE){
            if(TextUtils.isEmpty(appkey)) {
                Log.e("infoo","TextUtils.isEmpty(appkey)");
                YWAPI.init(mApp, APP_KEY);
            } else {
                //YWAPI.init(mApp, appkey);
                Log.e("infoo","else :TextUtils.isEmpty(appkey)");
                YWAPI.init(mApp, APP_KEY);
            }
        }
        else if(type== TcmsEnvType.TEST){
//            YWAPI.aliInit(mApp, APP_KEY_TEST, AccountUtils.SITE_CNHHUPAN);
            Log.e("infoo","type== TcmsEnvType.TEST");
            YWAPI.init(mApp, APP_KEY_TEST);
        }

        if (!TextUtils.isEmpty(userId) && !TextUtils.isEmpty(appkey)){
            Log.e("infoo","!TextUtils.isEmpty(userId) && !TextUtils.isEmpty(appkey)");
            LoginSampleHelper.getInstance().initIMKit("13872312561", APP_KEY);
        }
    ...
    private IYWTribePushListener mTribeListener = new IYWTribePushListener() {
        ...
        public void onPushMessage(YWTribe tribe, YWMessage message) {
            ...
            if(message.getSubType()==-1){
                ...
                msg.setTime(TimeUtil.getSystemCurrentTime());
                MessageInfo.addMessage(msg);
            }
