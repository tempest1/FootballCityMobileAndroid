package com.footballcitymobileandroid.openimui.demo;
...
import com.footballcitymobileandroid.openimui.sample.CustomConversationHelper;
import com.footballcitymobileandroid.openimui.sample.LoginSampleHelper;
import com.footballcitymobileandroid.openimui.tribe.TribeConstants;
...
public class FragmentTabs extends FragmentActivity {
    ...
    private void initCustomConversation() {
        ...
        CustomConversationHelper.addCustomViewConversation("myConversation","这个会话的展示布局可以自定义2");
    }
        ...
        mConnectionListener = new IYWConnectionListener() {
            ...
            public void onDisconnect(int code, String info) {
                if (code == YWLoginCode.LOGON_FAIL_KICKOFF) {
                    //在其它终端登录，当前用户被踢下线
                    do_exit();
//                    Toast toast = Toast.makeText(getApplicationContext(), "您的账号已在别的设备上登录", Toast.LENGTH_LONG);
//                    toast.show();
...
//                    DemoApplication.getContext().startActivity(intent);
    ...
    private void do_exit() {
        ...
        if (MainApplication.getUserInfo() != null) {
            appAction.fc_userExit(MainApplication.getUserInfo().getPhoneNumb(), MainApplication.getUserInfo().getPassWord(), Params.fc_userExit, exit);
...
//        }
    ...
    public ActionCallBackListener<BaseEntity<Void>> exit=new ActionCallBackListener<BaseEntity<Void>>(){
...
//            DemoApplication.getContext().startActivity(intent);
            ...
            intent.setClass(getApplicationContext(), CommonLogin.class);
